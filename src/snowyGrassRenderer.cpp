...
SnowyGrassRenderer::SnowyGrassRenderer(Loader loader, SnowyGrassShader snowy_grass_shader,
                             glm::mat4 projectionMatrix)
    : shader(snowy_grass_shader) {
  this->loader = loader;
  quad_chunks = new RawModel[xMapChunks * yMapChunks];
  std::string path =
      getCurrentWorkingDirectory() + "/resources/textures/snowy_grass_texture.png";
...
void SnowyGrassRenderer::unbind() {
...
void SnowyGrassRenderer::render(Camera camera) {
  shader.use();
...
void SnowyGrassRenderer::loadProjectionMatrix(glm::mat4 projectionMatrix) {
  shader.loadProjectionMatrix(projectionMatrix);
}

void SnowyGrassRenderer::setUpVAO(std::vector<float> snowy_grass_vertices, int idx) {
  // TODO: Implement setUpVAO
  RawModel quad = loader.loadToVAO(snowy_grass_vertices, 3);
  quad_chunks[idx] = quad;
}

unsigned int SnowyGrassRenderer::loadTextureFromFile(const char *path) {
...
std::string SnowyGrassRenderer::getCurrentWorkingDirectory() {
