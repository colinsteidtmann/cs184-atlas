...
WaterRenderer::WaterRenderer(Loader loader, WaterShader waterShader,
    ...
    for (int x = 0; x < xMapChunks; x++) {
      ...
      if (std::abs(gridPosX - x) <= chunk_render_distance &&
        ...
        glm::mat4 modelMatrix = createTransformationMatrix(
            glm::vec3(quad.x, quad.y, quad.z), 0, 0, 0, 1, 1);
...
void WaterRenderer::setUpVAO(std::vector<float> water_vertices, int idx,
                             float x, float y, float z) {
  // Just x and z vectex positions here, y is set to 0 in v.shader
  // We have all the water vertices, now make a quad out of them using the
  // min/max of the x and z.
